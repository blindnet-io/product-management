...
_**FR-FE08.** The Interface must allow a Data Consumer to send a Form to a Patient_

A Data Consumer sends a Form to a Patient in the following workflow:
* Step 1: A Data Consumer checks the checkbox of a Form she wishes to send. It is possible to select multiple Forms.
* Step 2: A Data Consumer inputs an email of a Patient
* Step 3: A Data Consumer clicks the send button and a request for emailing a Patient is sent to the Server (FR-BE05)
...
_**FR-FE09.** The Form must allow a Patient to fill out the data and to submit it encrypted to a Data Consumer._
...
When a Patient with the same email fills a Form for the second time for the same Data Consumer, new data will be saved in addition to the old data (a Data Consumer will have two form data for the same patient).

-------------------------------------------------------------------------------------------------

### Reading and managing the data (related to Search)

_**FR-FE10.** The Interface must allow a Data Consumer to see all Patients and their information (excluding the Medical data)._
...
_**FR-BE09.** The Server must be able to retrieve Metadata of Patients for a given Data Consumer after a given date._
...
_**FR-FE11.** The Interface must allow a Data Consumer to retrieve the Medical data for a given Patient, and to display it._
...
_**FR-FE12.** The Interface must allow a Data Consumer to delete the Medical data of a given Patient_
...
_**FR-FE13.** The Interface must allow a Data Consumer to update the Medical data of a Patient_
...
The link must lead to a GDPR page which is associated with the Data Consumer, where a Patient can make a GDPR related request to exercise his rights.
...
* A unique GDPR rights link that a patient is able to copy. The link will lead to a GDPR demand page specific to the Data Consumer.
...
_**FR-GDPR-06.** After submitting a form, the System must send an email to a patient, including a unique link to the GDPR rights page for the Data Consumer._
...
_**FR-GDPR-FE-06.** The Interface must allow a Patient to view his data once his view request has been approved by a Data Consumer._
...
The Data Consumer must also be able to add a textual message to the response, which must be encrypted for a Patient in the same way with blindnet SDK as the forms when the request is accepted (FR-GDPR-FE-05). This text field is mandatory for a rejection, since the Data Consumer must provide a reason.
...
Once the Server receives the data, it must create a unique link for the request response, and email that link to a Patient informing him about the rejection. Patient consumes Data Consumer’s response in the same way as he would consume the forms if the request was accepted (workflow in FR-GDPR-FE-06)

-------------------------------------------------------------------------------------------------

#### Modify

_The GDPR modify request workflows are the same as those for the view request. The difference is in the possibility for a patient to modify his data, and the option for the Data Consumer to manage modifications, as described in the FRs below._
...
When viewing the request, the Data Consumer must be able to see the list of all Patient’s forms. Each form must be clickable, and each list entry must contain: 
...
_**FR-GDPR-BE-10.** The System must delete selected Patient’s forms once a Data Consumer approves the GDPR delete request._

When the Data Consumer accepts the deletion, Data Consumer’s textual response (if any) is encrypted for the Patient (same as in other requests) and an email with a link is sent to a Patient to inform him about the decision and provide him a way to see the Data Consumer’s response.
...
If a request is rejected, the Data Consumer’s response and relevant request data are sent to the Server for storage (encrypted for the Data Consumer). Also, the response is encrypted for the Patient and the Patient is informed by email.

Figma ref: [4-2.4](https://www.figma.com/file/dkfknQnwNhCTGOMgIPjayx/PF-GDPR-Sketches?node-id=290%3A390)

_**FR-GDPR-FE-19.** The Interface must allow a Patient to view Data Consumer’s response on the GDPR delete request once the request has been treated by the Data Consumer._
...
Details include request submission date, acceptance/rejected date, Patient’s access date (if any), and a Data Consumer’s response (if any).
...
The results must consist only of Patients’ full names (name + surname), and are presented in a drop down list for a Data Consumer to choose. Once chosen, the full name becomes an input in the form search.
...
_**FR-FE20.** The Interface must be able to show both Data Consumer and Patients the fingerprint of Data Consumer’s public key._
...
Deleting an account implies deleting all Data Consumer’s data + all of her Patients’ forms and medical data.
...
2. Data Consumer approves the GDPR request to view or modify the data
    ...
    - f. If accepted:
        - i. SDK:
            ...
            - 2. Encrypts Data Consumer’s answer for a patient (FR-SDK05, FR-SDK05-2)
            - 3. Encrypts the answer for a Data Consumer (FR-SDK05, FR-SDK05-2)
        - ii. FE stores the encrypted form(s) for a Patient and Data Consumer’s encrypted answers in dedicated db tables
...
4. Patient receives a rejected/accepted response
    ...
    - e. FE shows the Data Consumer’s answer to a Patient
5. Patient views the data
    ...
    - e. FE shows the form(s) and Data Consumer’s answer to a Patient
...
8. Patient receives an email
    ...
    - c. FE shows the Data Consumer’s answer to a Patient
